...
func printQuickstart(cmd *cobra.Command) {
	...
	if ComponentsEnabled {
		// Full build quickstart with project management commands
		...
		// Core build quickstart - only pack operations
		_, _ = fmt.Fprintln(cmd.OutOrStdout(), p.Dim("This is epack-core (pack operations only)."))
		_, _ = fmt.Fprintln(cmd.OutOrStdout(), p.Dim("For collectors and tools, install the full version."))
		_, _ = fmt.Fprintln(cmd.OutOrStdout())
		_, _ = fmt.Fprintln(cmd.OutOrStdout(), p.Bold("Quick Start:"))
		_, _ = fmt.Fprintln(cmd.OutOrStdout())
		_, _ = fmt.Fprintln(cmd.OutOrStdout(), p.Dim("  # Build a pack from artifacts"))
		_, _ = fmt.Fprintln(cmd.OutOrStdout(), "  epack build evidence.epack ./data/*.json --stream myorg/prod")
		_, _ = fmt.Fprintln(cmd.OutOrStdout())
		_, _ = fmt.Fprintln(cmd.OutOrStdout(), p.Dim("  # Sign the pack (opens browser for authentication)"))
		_, _ = fmt.Fprintln(cmd.OutOrStdout(), "  epack sign evidence.epack")
		_, _ = fmt.Fprintln(cmd.OutOrStdout())
		...
		_, _ = fmt.Fprintln(cmd.OutOrStdout(), "  epack verify evidence.epack")
		_, _ = fmt.Fprintln(cmd.OutOrStdout())
		_, _ = fmt.Fprintln(cmd.OutOrStdout(), p.Dim("  # Inspect pack contents"))
		_, _ = fmt.Fprintln(cmd.OutOrStdout(), "  epack inspect evidence.epack")
		_, _ = fmt.Fprintln(cmd.OutOrStdout())
		_, _ = fmt.Fprintln(cmd.OutOrStdout(), p.Bold("Commands:"))
		_, _ = fmt.Fprintln(cmd.OutOrStdout(), "  build      Build an evidence pack from artifacts")
		_, _ = fmt.Fprintln(cmd.OutOrStdout(), "  sign       Sign an evidence pack")
