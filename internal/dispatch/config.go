...
func loadToolConfig(workDir, toolName string) (*config.JobConfig, config.ToolConfig, *lockfile.LockFile, string, error) {
	// Search upward for epack.yaml
	projectRoot, err := project.FindRoot(workDir)
	if err != nil {
		// epack.yaml not found - allow PATH fallback
		return nil, config.ToolConfig{}, nil, "", &configLoadError{notFound: true, err: err}
	}
	...
	if err != nil {
		...
		return nil, config.ToolConfig{}, nil, "", &configLoadError{notFound: false, err: fmt.Errorf("loading config: %w", err)}
	}

	toolCfg, ok := cfg.Tools[toolName]
	if !ok {
		// Tool not in config - allow PATH fallback (tool may be external)
		return nil, config.ToolConfig{}, nil, "", &configLoadError{notFound: true, err: fmt.Errorf("tool %q not configured", toolName)}
	}

	lf, err := lockfile.Load(lockfilePath)
	if err != nil {
		// Lockfile missing or invalid - this is a real error for configured tools
		return nil, config.ToolConfig{}, nil, "", &configLoadError{notFound: false, err: fmt.Errorf("loading lockfile: %w", err)}
	}

	return cfg, toolCfg, lf, projectRoot, nil
}
